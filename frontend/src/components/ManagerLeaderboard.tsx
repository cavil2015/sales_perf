...
export function ManagerLeaderboard({ managers, loading }: ManagerLeaderboardProps) {
  return (
      ...
      {loading && managers === null ? (
        ...
        <div className="space-y-3">
          {managers.slice(0, 7).map((m) => (
            <div key={m.managerId} className="flex items-center justify-between p-2 hover:bg-slate-50 rounded-lg transition-colors">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="font-medium text-sm truncate" title={m.managerName}>{m.managerName}</p>
                  <p className="text-xs text-slate-500">{m.salesCount} deals</p>
                </div>
              </div>
              <div className="text-right">
                <p className="font-semibold text-sm">{formatCurrency(m.grossProfit)}</p>
              </div>
            </div>
          ))}
        </div>
