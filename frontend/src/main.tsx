...
if (import.meta.env.PROD && typeof window !== 'undefined') {
    ...
    if (typeof devToolsHook === 'object' && devToolsHook !== null) {
        for (const key in devToolsHook) {
            ...
            else if (typeof devToolsHook[key] === 'function') devToolsHook[key] = () => {};
    ...
    const observer = new PerformanceObserver((list) => {
      for (const entry of list.getEntries()) {
        ...
        if (entry.duration > 50) {
          console.warn(`[Performance] Long Task detected: ${entry.duration.toFixed(0)}ms`, entry);
        }
      }
    });
      ...
      return (
        <div className="flex h-screen w-full items-center justify-center bg-gray-50 p-6">
          <div className="rounded-xl bg-white p-8 shadow-lg max-w-lg w-full border-t-4 border-red-500">
            <h2 className="text-2xl font-bold text-slate-900 mb-4">Application Crashed</h2>
            <p className="text-slate-600 mb-4">A critical rendering error occurred. Please refresh the page.</p>
...
if (!rootElement) {
    document.body.innerHTML = '<div style="color:red;padding:20px;text-align:center;font-family:sans-serif;">Fatal Error: Root DOM element missing. Check index.html.</div>';
