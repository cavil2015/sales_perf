...
function buildUrl(endpoint: string, from?: string, to?: string) {
    ...
    return qs ? `${cleanBase}${cleanEndpoint}?${qs}` : `${cleanBase}${cleanEndpoint}`;
}
...
function createTimeoutSignal(timeoutMs: number, customSignal?: AbortSignal): AbortSignal {
    ...
    if (customSignal) {
        ...
        else customSignal.addEventListener('abort', () => controller.abort(customSignal.reason));
    }

    // Set the fallback timeout
    const timeoutId = setTimeout(() => controller.abort(new Error('API Timeout')), timeoutMs);
...
export const fetchKpis = async (from?: string, to?: string, signal?: AbortSignal) => {
...
export const fetchManagersRating = async (from?: string, to?: string, signal?: AbortSignal) => {
  const res = await fetchWithRetry(buildUrl('/analytics/managers-rating', from, to), { 
      signal,
      credentials: 'include',
      headers: { 
          'Accept': 'application/json',
...
export const fetchChartData = async (from?: string, to?: string, signal?: AbortSignal) => {
